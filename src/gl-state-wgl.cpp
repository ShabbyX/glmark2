...
GLStateWGL::GLStateWGL() : wgl_library_(0), hdc_(0), wgl_context_(0), get_proc_addr_(0) {}
...
{
    wgl_library_ = LoadLibraryA("opengl32.dll");
    if (!wgl_library_) {
    ...
    get_proc_addr_ = GetProcAddress(wgl_library_, "wglGetProcAddress");
    if (!get_proc_addr_) {
...
GLStateWGL::api_proc GLStateWGL::load_proc(const char *name, void *userptr)
{
    ...
    FARPROC proc = GetProcAddress(state->wgl_library_, name);
    if (proc) {
